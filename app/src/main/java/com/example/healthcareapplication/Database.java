...
public class Database extends SQLiteOpenHelper {
    ...
    public int login(String username,String password){
        ...
        @SuppressLint("Recycle") Cursor c = db.rawQuery("select * from user where username=? and password=?",str);
        if (c.moveToFirst()){
            result=1;
        }
        return result;
    ...
    public ArrayList getOrderData(String username) {
        ...
        if (c.moveToFirst()) {
            do {
                arr.add(c.getString(1) + "$" + c.getString(2) + "$" + c.getString(3) + "$" + c.getString(4) + "$" + c.getString(5)+"$" + c.getString(7) + "$" + c.getString(8));
            } while (c.moveToNext());
        }
        db.close();
        return arr;
